...
def project2d(points, m):
    d0 = np.array([0, 0, 0, 0, 0], np.float32)
    pts, jac = cv2.projectPoints(points, np.array(
        [0, 0, 0], np.float32), np.array([0, 0, 0], np.float32), m, d0)
...
def project3d(point, m):
    ...
    return np.int32(p.transpose()/p.transpose()[..., 2])[:, :2]


if __name__ == "__main__":
    fn = "test/projection/intrinsics.yml"
    m = getMatrix(fn)
    print(m)
